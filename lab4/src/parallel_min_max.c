#include <ctype.h>
#include <limits.h>
#include <stdbool.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <signal.h>

#include <sys/time.h>
#include <sys/types.h>
#include <sys/wait.h>

#include <getopt.h>

#include "find_min_max.h"
#include "utils.h"

pid_t *child_pids;  // Массив для хранения PID дочерних процессов
int num_children;   // Количество дочерних процессов

void kill_children() {
    for (int i = 0; i < num_children; i++) {
        if (child_pids[i] > 0) {
            kill(child_pids[i], SIGKILL);
        }
    }
}

void timeout_handler(int signum) {
    printf("\nTimeout reached! Killing all child processes...\n");
    kill_children();
    exit(1);
}

int main(int argc, char **argv) {
  int seed = -1;
  int array_size = -1;
  int pnum = -1;
  int timeout = -1;  // Добавляем переменную для таймаута
  bool with_files = false;

  while (true) {
    int current_optind = optind ? optind : 1;

    static struct option options[] = {{"seed", required_argument, 0, 0},
                                    {"array_size", required_argument, 0, 0},
                                    {"pnum", required_argument, 0, 0},
                                    {"timeout", required_argument, 0, 0},  // Добавляем опцию таймаута
                                    {"by_files", no_argument, 0, 'f'},
                                    {0, 0, 0, 0}};

    int option_index = 0;
    int c = getopt_long(argc, argv, "f", options, &option_index);

    if (c == -1) break;

    switch (c) {
      case 0:
        switch (option_index) {
          case 0:
            seed = atoi(optarg);
            if (seed <= 0) {
                printf("seed is a positive number\n");
                return 1;
            }
            break;
          case 1:
            array_size = atoi(optarg);
            if (array_size <= 0) {
                printf("array_size is a positive number\n");
                return 1;
            }
            break;
          case 2:
            pnum = atoi(optarg);
            if (pnum <= 0) {
                printf("pnum is a positive number\n");
                return 1;
            }
            break;
          case 3:  // Обработка таймаута
            timeout = atoi(optarg);
            if (timeout <= 0) {
                printf("timeout is a positive number\n");
                return 1;
            }
            break;
          case 4:
            with_files = true;
            break;
          default:
            printf("Index %d is out of options\n", option_index);
        }
        break;
      case 'f':
        with_files = true;
        break;
      case '?':
        break;
      default:
        printf("getopt returned character code 0%o?\n", c);
    }
  }

  if (optind < argc) {
    printf("Has at least one no option argument\n");
    return 1;
  }

  if (seed == -1 || array_size == -1 || pnum == -1) {
    printf("Usage: %s --seed \"num\" --array_size \"num\" --pnum \"num\" [--timeout \"num\"] \n",
           argv[0]);
    return 1;
  }

  int *array = malloc(sizeof(int) * array_size);
  GenerateArray(array, array_size, seed);
  int active_child_processes = 0;

  // Выделяем память для массива PID дочерних процессов
  child_pids = malloc(sizeof(pid_t) * pnum);
  num_children = pnum;

  struct timeval start_time;
  gettimeofday(&start_time, NULL);

  // Устанавливаем обработчик таймаута, если он задан
  if (timeout > 0) {
    signal(SIGALRM, timeout_handler);
    alarm(timeout);
  }

  int part_size = array_size / pnum;
  int pipefd[pnum][2];
  FILE *files[pnum];

  for (int i = 0; i < pnum; i++) {
    if (!with_files) {
      if (pipe(pipefd[i]) == -1) {
        printf("Pipe failed!\n");
        return 1;
      }
    } else {
      char filename[20];
      sprintf(filename, "min_max_%d.txt", i);
      files[i] = fopen(filename, "w+");
      if (files[i] == NULL) {
        printf("File opening failed!\n");
        return 1;
      }
    }

    pid_t child_pid = fork();
    if (child_pid >= 0) {
      active_child_processes += 1;
      if (child_pid == 0) {
        // Дочерний процесс
        struct MinMax min_max = GetMinMax(array, i * part_size,
                                        (i == pnum - 1) ? array_size : (i + 1) * part_size);

        if (with_files) {
          fprintf(files[i], "%d %d", min_max.min, min_max.max);
          fclose(files[i]);
        } else {
          close(pipefd[i][0]);
          write(pipefd[i][1], &min_max, sizeof(struct MinMax));
          close(pipefd[i][1]);
        }
        return 0;
      } else {
        // Родительский процесс
        child_pids[i] = child_pid;  // Сохраняем PID дочернего процесса
      }
    } else {
      printf("Fork failed!\n");
      return 1;
    }
  }

  while (active_child_processes > 0) {
    wait(NULL);
    active_child_processes -= 1;
  }

  // Отключаем таймер, если он был установлен
  if (timeout > 0) {
    alarm(0);
  }

  struct MinMax min_max;
  min_max.min = INT_MAX;
  min_max.max = INT_MIN;

  for (int i = 0; i < pnum; i++) {
    int min = INT_MAX;
    int max = INT_MIN;

    if (with_files) {
      char filename[20];
      sprintf(filename, "min_max_%d.txt", i);
      files[i] = fopen(filename, "r");
      fscanf(files[i], "%d %d", &min, &max);
      fclose(files[i]);
      remove(filename);
    } else {
      close(pipefd[i][1]);
      struct MinMax temp;
      read(pipefd[i][0], &temp, sizeof(struct MinMax));
      close(pipefd[i][0]);
      min = temp.min;
      max = temp.max;
    }

    if (min < min_max.min) min_max.min = min;
    if (max > min_max.max) min_max.max = max;
  }

  struct timeval finish_time;
  gettimeofday(&finish_time, NULL);

  double elapsed_time = (finish_time.tv_sec - start_time.tv_sec) * 1000.0;
  elapsed_time += (finish_time.tv_usec - start_time.tv_usec) / 1000.0;

  free(array);
  free(child_pids);  // Освобождаем память массива PID

  printf("Min: %d\n", min_max.min);
  printf("Max: %d\n", min_max.max);
  printf("Elapsed time: %fms\n", elapsed_time);
  fflush(NULL);
  return 0;
}